...
/*  globalAccountSettings must be an array (can be undefined if you use
    globalNetworkCheckSettings or globalNetworkAccountSettings)

    * the href value is a "principal URL" - the last character in href must
      be '/'
      principal URL != collection URL -> the client automatically detects
      collections for each principal URL
    ...
    * the hrefLabel sets the server name in the resource header - useful if you
      want to see custom resource header above the collections; you can use the
      following variables:
    ...
    * the forceReadOnly sets the resource or list of collections as "read-only"
      - if true then the whole resource will be "read-only";
        if an array of URL encoded collections or regexes
          (for example: ['/caldav.php/user/calendar/',
        ...
        if null (default), unset or unknown then server detected privileges
          are used

    * the timeOut sets the timeout for jQuery .ajax call (in miliseconds)

    * the lockTimeOut sets the LOCK Timeout value if resource locking is used
      (in miliseconds)

    * the settingsAccount sets the account where client properties are saved
      during logout and resource/collection synchronisation (note: set it to
      true only for ONE account)

    * the checkContentType enables content-type checking for server response
      (only objects with proper content-type are inserted into interface)
      - if you cannot see data in the interface you may try to disable it
          (useful if your server return wrong value in
            "propstat/prop/getcontenttype");
        if undefined content-type checking is enabled

    * the delegation allows user to load delegated (shared) resources
      - if true (default) then delegation is enabled and the interface allows
          you to select which delegated resources/collections you want to load;
        if false then delegation is disabled

    * the ignoreAlarms defines an array of calendars with disabled alarm
      functionality
      - if true then alarm functionality is disabled for all collections;
        if false (default) then alarm functionality is enabled for all
          collections;
    ...
    * the backgroundCalendars defines an array of background calendars
      - if there is at least one event defined for the given day in a
        background calendar, the background color for that day will be
        pink/light-red; to use this feature define an array of URL encoded
        collections or regexes (for example:
          ['/caldav.php/user/calendar/',
            '/caldav.php/user%40domain.com/calendar/',
            new RegExp('^/caldav.php/user/calendar[0-9]/$', 'i')])

    special options not present in the default config
    (use only if you know what are you doing!):

    * the crossDomain sets jQuery ajax crossDomain value
      (use only if you know what are you doing!)
      - by default null = autodetect /detected setting is shown in the console/

    * the withCredentials sets jQuery's ajax withCredentials value for cross
      domain queries (use only if you know what are you doing!)
...
//     backgroundCalendars: []
//   },
//   { href: 'https://server1.com:8443/principals/users/USERNAME2/',
//     hrefLabel: null,
//     forceReadOnly: null,
//     settingsAccount: false,
//     checkContentType: true,
//     userAuth: {userName: 'USERNAME2', userPassword: 'PASSWORD2'},
//     timeOut: 90000,
//     lockTimeOut: 10000,
//     delegation: true,
//     ignoreAlarms: false,
...
/*  if set, the client authenticates against the href URL (the last character
    in href must be '/') and if the authentication is successful it appends the
    USER + '/' to end of href and sets the userAuth:
      {userName: USER, userPassword: PASSWORD}
    then the client uses the modified globalNetworkCheckSettings in the same
    way as the globalAccountSettings

    this option ivokes a login screen and disallows access until successfull
    authentication

    * the additionalResources array can contain additional resources
      (shared resources accessible by all users), for example:
        additionalResources: ['company','customers'] ...
      href values for these resources are created in the same way as described
      above for the USER

    see globalAccountSettings (above) comments for more information          */
/*  Lion server example (http + https setup; see misc/readme_lion.txt for
    server setup):                                                           */
// var globalNetworkCheckSettings={
//   href: 'http://lion.server.com:8008/principals/users/',
//   hrefLabel: null,
//   additionalResources: [],
//   forceReadOnly: null,
//   settingsAccount: true,
...
// }
// var globalNetworkCheckSettings={
//   href: 'https://lion.server.com:8443/principals/users/',
//   hrefLabel: null,
//   additionalResources: [],
//   forceReadOnly: null,
//   settingsAccount: true,
...
// var globalNetworkCheckSettings={
//   href: 'http://davical.server.com:8080/caldav.php/',
//   hrefLabel: null,
//   additionalResources: [],
//   forceReadOnly: null,
//   settingsAccount: true,
...
//   ignoreAlarms: false
// }
/* Davical example (client installed into Davical subdirectory - works out of
   the box, no additional setup required):                                   */
var globalNetworkCheckSettings={
...
/*  if set, the configuration is loaded from the network
    (using HTTP basic auth) - the returned configuration XML settings are added
    to globalAccountSettings ... it is possible to combine this option with the
    globalAccountSettings although it is not recommended

    this option invokes a login screen and disallows access until the client
    gets correct XML configuration file from the server

    * the timeOut sets the timeout for jQuery .ajax call (in miliseconds)    */
// var globalNetworkAccountSettings={
//   href: 'https://www.config-server.com/auth/',
//   timeOut: 30000};
/*  default configuration if the auth module is located in the currect
    subdirectory                                                             */
// var globalNetworkAccountSettings={
//   href: location.protocol
//         +'//'
...
/*  if defined and not empty then only languages listed here are shown at the
    login screen (for example: ['en_US', 'sk_SK']),
    otherwise (default) all languages are shown values in the array must refer
    to an existing localization defined in the common.js
    (see the option above)                                                   */
var globalInterfaceCustomLanguages=[];

/*  JavaScript localeCompare() or custom alphabet for data sorting
    custom alphabet is used by default because the JavaScript localeCompare()
    not support collation and often returns "wrong" result                   */
//var globalSortAlphabet=null;  // use localeCompare()
var globalSortAlphabet=' '
  +'0123456789'
  +'AÀÁÂÄÆÃÅĀ'+'B'+'CÇĆČ'+'DĎ'+'EÈÉÊËĒĖĘĚ'+'F'+'GĞ'+'H'+'IÌÍÎİÏĪĮ'+'J'+'K'
  +'LŁĹĽ'+'M'+'NŃÑŇ'+'OÒÓÔÖŐŒØÕŌ'+'P'+'Q'+'RŔŘ'+'SŚŠȘșŞşẞ'+'TŤȚțŢţ'+'UÙÚÛÜŰŮŪ'
  +'V'+'W'+'X'+'YÝŸ'+'ZŹŻŽ'
  +'aàáâäæãåā'+'b'+'cçćč'+'dď'+'eèéêëēėęě'+'f'+'gğ'+'h'+'iìíîïīįı'+'j'+'k'
  +'lłĺľ'+'m'+'nńñň'+'oòóôöőœøõō'+'p'+'q'+'rŕř'+'sśšß'+'tť'+'uùúûüűůū'+'v'+'w'
  +'x'+'yýÿ'+'zźżž'
  +'АБВГҐДЕЄЖЗИІЇЙКЛМНОПРСТУФХЦЧШЩЮЯЬ'+'абвгґдеєжзиіїйклмнопрстуфхцчшщюяь';
    // use custom alphabet sorting (note: the first character is "space")
/* search functionality character equivalence
   (transformation to ASCII: key = regex text, value = result character)     */
...
/*  update notification will be shown only to users with login names defined in
    this array (for example: ['admin', 'peter'])
...
/*  set the datepicker format
    (see http://docs.jquery.com/UI/Datepicker/formatDate for valid values)
    note: date format is now predefined for each localization - use this option
    only if you want to use custom date format instead of the predefined one */
...
/*  if more than one resource (server account) is configured, sort resources
    alphabetically?                                                          */
var globalResourceAlphabetSorting=true;

/*  asynchronously sync resources/collections on background
    every X miliseconds                                                      */
var globalSyncResourcesInterval=120000;

/*  enable background sync even if browser window has no focus
    (if false, sync is performed only if browser window/tab is focused)
...
/*  where to store user settings such as: active view, enabled/selected
    collections, ... (we store them into DAV property on the server)

    note: not all servers support storing DAV properties (some servers support
    only subset /or none/ of these URLs)

    if 'principal-URL', '', null or undefined (default) - settings are stored
      to principal-URL
...
/*  settings such as enabled/selected collections are stored on the server
    (see the previous option) in form of full URL
      (e.g.: http://user@server:port/principal/collection/),
    but even if this approach is "correct" (you can use the same principal URL
    with multiple different logins, ...) it causes a problem if your server is
    accessible from multiple URLs (e.g. http://server/ and https://server/).
    If you want to store only the "principal/collection" part of the URL
    (instead of full URL) then enable this option                            */
