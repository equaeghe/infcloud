								...
								for(var u=0;u<alertTime.length;u++)
								{
									...
									if(aTime>now)
									{
										...
										alertTimeOut[alertTimeOut.length]=setTimeout(function(){
												showAlertEvents(inputEvent.uid, (aTime-now),{start:start, allDay:all, title:title});
										}, delay);
									}
								}
						}
											...
											if((alertTime[v].charAt(0)=='-') || (alertTime[v].charAt(0)=='+'))
											{
												...
												if(aTime>now)
												{
													...
													alertTimeOut[alertTimeOut.length]=setTimeout(function(startTime){
														showAlertEvents(inputEvent.uid, (aTime-now), {start:new Date(startTime.getTime()), allDay:all, title:title});
													}, delay,startTime);
												}
											}
										}
...
// FN (extracted from newly created N [previous "process_elem"], required by RFC)
	...
	// if globalSettings.contactstorefn.value is missing use globalCollectionDisplay
	if(globalSettings.contactstorefn.value==undefined)
		globalSettings.contactstorefn.value=globalSettings.collectiondisplay.value;
...
// PHOTO
	...
	{
		var value = tmpvCardEditorRef.find('#photo').get(0).toDataURL('image/png');
...
//console.time('PHOTO timer');
		...
		if(globalDisabledContactAttributes.indexOf('PHOTO')==-1)
		{
			...
			if(vcard_element!=null)	// if the PHOTO attribute is present more than once, we use the first value
			{
				...
				var re=RegExp('TYPE=(.*)','i');
				for(i=1;i<parsed_value.length;i++)
					if((type_value=parsed_value[i].match(re))!=undefined)
					{
				...
				vCard.tplM['contentline_PHOTO'][0]=vCard.tplM['contentline_PHOTO'][0].replace('##:::##params_wsc##:::##', parsed[3]);
...
// TODO: move regex object directly into config.js
				...
				tmpvCardEditorRef.find('[data-type="\\%address"]:eq('+element_i+') [data-addr-field="street"]').val(vcardUnescapeValue(parsed_value[2]));
