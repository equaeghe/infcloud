			...
			for (var col, c=0; c < tableCols.length; c++) {
				...
				if (col == 'handle') {
					...
					s += "<td class='fc-event-check'>" + '<input type="checkbox" class="fc-event-checkbox" data-ind="false"/>' + "</td>";
						...
						s += "<td class='fc-event-priority fc-event-priority-" + event.renderPriority + "'></td>";
				} else if (col == 'time') {
						s += "<td class='fc-event-time'>" + htmlEscape(dueTime) + "</td>";
				} else if (col == 'title') {
					s += "<td class='fc-event-title'>" + (event.title ? htmlEscape(event.title.replace(/(\r\n|\n|\r)+/gm," ")) : '&nbsp;') + "</td>";
				} else if (col == 'location') {
						s += "<td class='fc-event-location'>" + (event.location ? htmlEscape(event.location.replace(/(\r\n|\n|\r)+/gm," ")) : '&nbsp;') + "</td>";
				} else if (col == 'status') {
...
function TodoView(element, calendar) {
	...
	function setWidth(width) {
		element.width(width);
		var slots = Math.floor((width - dateInfo.outerWidth()) / datepickers[0].outerWidth());
		...
		opt('todoColThresholds').forEach(function(e){
			var column = $('col.fc-event-'+e.col);
			if(width<e.width && column.css('display')!=='none')
				$('.fc-event-'+e.col).css('display', 'none')
			else if(width>=e.width && column.css('display')==='none')
				$('.fc-event-'+e.col).css('display', '')
		});
	}
